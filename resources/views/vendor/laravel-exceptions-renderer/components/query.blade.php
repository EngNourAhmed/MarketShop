...
>
    ...
    <div class="flex flex-col gap-1">
        @php
            $queriesSlice = array_slice($queries, 0, 100);
        @endphp
        @if (count($queriesSlice) > 0)
            @foreach ($queriesSlice as $queryIndex => $queryRow)
                <div
                    ...
                    x-show="Math.floor({{ $queryIndex }} / perPage) === (currentPage - 1)"
                >
                    <div class="flex items-center gap-2 truncate">
                        <div class="flex items-center gap-2">
                            <x-laravel-exceptions-renderer::icons.database class="w-3 h-3 text-neutral-500 dark:text-neutral-400" />
                            <span class="text-neutral-500 dark:text-neutral-400">{{ $queryRow['connectionName'] ?? '' }}</span>
                        </div>
                        <x-laravel-exceptions-renderer::syntax-highlight
                            ...
                            truncate
                            class="min-w-0"
                            data-tippy-content="{{ nl2br($queryRow['sql'] ?? '') }}"
                        />
                    </div>
                    <div class="text-neutral-500 dark:text-neutral-200 text-right flex-shrink-0">{{ $queryRow['time'] ?? 0 }}ms</div>
                </div>
            @endforeach
        @else
